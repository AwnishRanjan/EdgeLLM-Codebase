...
class QuantizeLinear(nn.Linear):
    ...
    def forward(self, input_):
        ...
        if self.w_bits >= 32:
            ...
            weight_clip_val = torch.tensor([-2.0, 2.0], device=real_weights.device, dtype=real_weights.dtype)
        ...
        # Quantize inputs
        if self.a_bits < 32 and self.a_bits > 2:
            act_clip_val = torch.tensor([-2.0, 2.0], device=input_.device, dtype=input_.dtype)
