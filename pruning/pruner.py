...
def get_pruned_model(model, tokenizer, args, logger):
    ...
    dataloader, _ = get_wikitext2(args.num_pruning_samples, 619, seqlen, model, tokenizer)
    time1 = time.time()
    if args.pruning_scheme == "unstructure":
        if args.layer_wise_pruning:
            if args.mse_file is None:
                raise ValueError("mse_file is required when layer_wise_pruning is enabled.")
            ...
            layer_pruning_ratios = {f"{i}":args.total_pruning_ratio  
                                        for i in range(len(model.model.layers))}
    ...
    model = llama_sequential(model, dataloader, args.pruning_device, args.num_pruning_samples, layer_pruning_ratios, logger, args.pruning_scheme, seqlen)
