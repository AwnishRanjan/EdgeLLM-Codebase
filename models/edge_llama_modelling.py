...
try:
    ...
    def _prepare_4d_causal_attention_mask(attention_mask, input_shape, inputs_embeds, past_key_values_length):
        return AttentionMaskConverter._prepare_4d_attention_mask(attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1])
...
class LlamaMLP(nn.Module):
    def __init__(self, config, a_bits: int, w_bits: int):
        self.w_bits = w_bits
        self.a_bits = a_bits
        super().__init__()
        self.config = config
        ...
        if self.config.pretraining_tp > 1:
            ...
            if "adapter_activation" in kwargs:
                if model_status == "eval":
                    ...
                    kwargs["model_status"] = "eval"
                    # cauculate the lora hidden states and non-lora hidden states
                    gate_proj, lora_gate_proj = self.gate_proj(x, **kwargs)
                    up_proj, lora_up_proj = self.up_proj(x, **kwargs)

                    temp_hidden_states = self.act_fn(gate_proj * up_proj)
                    temp_lora_hidden_states = self.act_fn(lora_gate_proj * lora_up_proj)

                    kwargs["lora_hidden_states"] = temp_lora_hidden_states

                    down_proj, lora_down_proj= self.down_proj(temp_hidden_states, **kwargs)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, a_bits: int, w_bits: int, kv_bits: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.kv_bits < 32:
            ...
            if model_status == "eval":
                lora_key_states = self.act_quantizer_k.apply(
                    lora_key_states, self.act_clip_val_k, self.kv_bits, False
                )
                lora_value_states = self.act_quantizer_k.apply(
                    lora_value_states, self.act_clip_val_k, self.kv_bits, False
                )
        ...
        if self.config.pretraining_tp > 1:
            ...
            if "adapter_activation" in kwargs:
                if model_status == "eval":
                    kwargs["lora_hidden_states"] = lora_attn_output
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, qats: dict()):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.qats = qats
        self.a_bits = qats["a"]
        self.w_bits = qats["w"]
        self.kv_bits = qats["kv"]
        self.self_attn = (
            LlamaAttention(config=config, a_bits=self.a_bits, w_bits=self.w_bits, kv_bits=self.kv_bits)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, layer_qats):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size

        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([LlamaDecoderLayer(config, layer_qats[i]) for i in range(config.num_hidden_layers)])
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config,layer_qats={i: {"w": 8, "a":8, "kv": 8} for i in range(32)}):
        super().__init__(config)
        self.model = LlamaModel(config, layer_qats = layer_qats)
        self.vocab_size = config.vocab_size
