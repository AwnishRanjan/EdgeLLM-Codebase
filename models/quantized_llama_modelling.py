...
class LlamaDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.self_attn = LlamaAttention(config=config, a_bits=self.a_bits, w_bits=self.w_bits, kv_bits=self.kv_bits)
        self.mlp = LlamaMLP(
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, 
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, layer_qats[i]) for i in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            seq_length_with_past = 16 + past_key_values_length
        ...
        # print(attention_mask.shape, seq_length, inputs_embeds.shape, past_key_values_length)
